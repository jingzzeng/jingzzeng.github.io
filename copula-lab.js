...
function update(regenerate=true){
 if(!$('seed').checkValidity()||!$('df').checkValidity()){$('error').textContent='请输入有效的整数：种子 1–2147483647，自由度 2–50。';return;}$('error').textContent='';
 const f=$('family').value,p=+$('param').value,df=+$('df').value,n=+$('n').value,seed=+$('seed').value,mx=$('x').value,my=$('y').value;
 $('param-value').textContent=p.toFixed(2);
 if(regenerate)uv=sample(n,f,p,df,seed);xy=uv.map(([u,v])=>[quantile(u,mx),quantile(v,my)]);
 if(!xy.every(p=>p.every(Number.isFinite))){$('error').textContent='数值计算失败，请调整参数或随机种子。';return;}
...
for(const id of ['param','df','n','seed'])$(id).addEventListener('input',()=>update());
for(const id of ['x','y'])$(id).addEventListener('change',()=>update(false));
$('resample').addEventListener('click',()=>{$('seed').value=(+$('seed').value%2147483646)+1;update();});
function preset(name){$('family').value='gaussian';familySetup();$('param').value=name==='normal'?.9:.5;$('x').value=name==='uniform'?'uniform':name==='mixed'?'t':'normal';$('y').value=name==='uniform'?'uniform':'normal';$('n').value='1000';$('seed').value='12345';$('df').value='4';update();}
